			...
			if(sort === 'swiper') {
				...
				if(this.checkValidate(target, option)){
					if(option.hasOwnProperty('pagination')){
						$(option.pagination.el).show();
					}
					if(option.hasOwnProperty('navigation')){
						$(option.navigation.nextEl).show();
						$(option.navigation.prevEl).show();
					}
					swiper = new Swiper(target, option);
				}else{ //제한 ok
					if(option.hasOwnProperty('pagination')){
						$(option.pagination.el).hide();
					}
					if(option.hasOwnProperty('navigation')){
						$(option.navigation.nextEl).hide();
						$(option.navigation.prevEl).hide();
					}
					swiper = null;
				}
				return swiper;
