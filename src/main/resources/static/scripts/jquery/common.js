...
//로그인 화면 개편에 따른 라디오버튼 클릭 이벤트 변경 처리
if($('.layer-login .detail-radio').length) {
    $('.layer-login .detail-radio').on('change', '.detail-radio-box input', function(e){
        if(e.target.value === 'P'){
            $('.layer-login .detail-tab-wrap-P').addClass('active');
        }
        else{
            $('.layer-login .detail-tab-wrap-P').removeClass('active');
        }
    });
    $('.layer-login .detail-radio').on('change', '.detail-radio-box input', function(e){
        if(e.target.value === 'L'){
            $('.layer-login .detail-tab-wrap-L').addClass('active');
        }
        else{
            $('.layer-login .detail-tab-wrap-L').removeClass('active');
        }
    });
}
...
/* 레이어팝업start-------------------------------------------------*/
    ...
    // 친구추천 초대장
    if($('.layer-agreement').length) {
    	MUI.layer.openClick('.layer-agreement-open', LAYER_DIM, LAYER_PARENT, true, function(show, layer){
    		show();
    	});
    }
...
$(document).ready(function () {
    locationB = $(".path-wrap .inner li a");
    locationB.css('cursor', 'default');
